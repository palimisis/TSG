    ...
    if self.ext == '.npy':
      if self.db_path == 'data/coco_pred_sg':
        self.loader = lambda x: np.load(
            x, allow_pickle=True, encoding="latin1").item()
      else:
...
class Dataset(data.Dataset):
  ...
  def collate_func(self, batch, split):
    ...
    for sample in batch:
      ...
      tmp_rela_seq = np.zeros(
          [len(rela_matrix)+len(attr_matrix)*2], dtype='int')
    ...
    fc_batch, att_batch, rela_batch, label_batch, gts, infos = \
        zip(*sorted(zip(fc_batch, att_batch, rela_batch,
            label_batch, gts, infos), key=lambda x: 0, reverse=True))
    data = {}
    ...
    for i in range(len(att_batch)):
      ...
      for j in range(rela_len[i]):
        data['enc_self_masks'][i, np.int32(
            rela_info[i][j][0]), att_batch[i].shape[0]+attr_len[i]+j] = 1
        data['enc_self_masks'][i, att_batch[i].shape[0] +
                               attr_len[i]+j, np.int32(rela_info[i][j][0])] = 1
        data['enc_self_masks'][i, np.int32(
            rela_info[i][j][1]), att_batch[i].shape[0]+attr_len[i]+j] = 1
        data['enc_self_masks'][i, att_batch[i].shape[0] +
                               attr_len[i]+j, np.int32(rela_info[i][j][1])] = 1
...
class DataLoader:
  def __init__(self, opt):
    ...
    for split in ['train', 'val', 'test']:
      if split == 'train':
        sampler = MySampler(
            self.dataset.split_ix[split], shuffle=False, wrap=True)
      else:
        sampler = MySampler(
      ...
      self.loaders[split] = data.DataLoader(dataset=self.dataset,
                                            ...
                                            collate_fn=lambda x: self.dataset.collate_func(
                                                x, split),
                                            drop_last=False)
