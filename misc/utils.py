...
class StructureLosses(nn.Module):
  ...
  def forward(self, input, seq, data_gts):
    ...
    if self.opt.entropy_reward_weight > 0:
      ...
      scores = scores + self.opt.entropy_reward_weight * \
          entropy.view(-1, seq_per_img)
    # rescale cost to [0,1]
    costs = - scores
...
class ReduceLROnPlateau(object):
  ...
  def __init__(self, optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08):
    self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode, factor, patience, threshold, threshold_mode, cooldown, min_lr, eps, verbose)
    self.optimizer = optimizer
...
def COS_dis(M, X):
  ...
  cosine_dis = 1 - torch.matmul(A_normalized.squeeze(),
                                B_normalized.squeeze().transpose(0, 1))
