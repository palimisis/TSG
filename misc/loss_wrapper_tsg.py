  ...
  def forward(self, fc_feats, att_feats, labels, masks, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len, gts, gt_indices,
    ...
    if struc_flag:
      if opt.structure_loss_weight < 1:
        lm_loss = self.crit(self.model(fc_feats, att_feats, labels, att_masks, enc_self_masks,
                            rela_seq, rela_len, attr_len, att_len), labels[..., 1:], masks[..., 1:])
      else:
      ...
      loss = (1-opt.structure_loss_weight) * lm_loss + \
          opt.structure_loss_weight * struc_loss['loss']
      out['lm_loss'] = lm_loss
