...
class EncoderDecoder(nn.Module):
  ...
  def decode(self, memory, src_tgt_mask, tgt, tgt_mask, rela_len, attr_len, att_len):
    memory_att, memory_attr, memory_rela, mask_att, mask_attr, mask_rela = divide_memory(
        memory, rela_len, attr_len, att_len)
    out = self.decoder(self.tgt_embed(tgt), memory_att, memory_attr,
                       memory_rela, mask_att, mask_attr, mask_rela, tgt_mask)
    return out
...
def divide_memory(memory, rela_len, attr_len, att_len):
  ...
  mask_att = torch.zeros([memory.shape[0], max_att_len],
                         requires_grad=False).cuda()
  mask_attr = torch.zeros(
      [memory.shape[0], max_attr_len], requires_grad=False).cuda()
  mask_rela = torch.zeros(
      [memory.shape[0], max_rela_len], requires_grad=False).cuda()

  memory_att = torch.zeros(
      [memory.shape[0], max_att_len, memory.shape[2]]).cuda()
  memory_attr = torch.zeros(
      [memory.shape[0], max_attr_len, memory.shape[2]]).cuda()
  memory_rela = torch.zeros(
      [memory.shape[0], max_rela_len, memory.shape[2]]).cuda()
  ...
  for i in range(len(rela_len)):
    ...
    memory_att[i*seq_per_img:(i+1)*seq_per_img, :att_len[i],
               :] = memory[i*seq_per_img:(i+1)*seq_per_img, :att_len[i], :]
    memory_attr[i*seq_per_img:(i+1)*seq_per_img, :attr_len[i], :] = memory[i *
                                                                           seq_per_img:(i+1)*seq_per_img, att_len[i]:attr_len[i]+att_len[i], :]
    memory_rela[i*seq_per_img:(i+1)*seq_per_img, :rela_len[i], :] = memory[i*seq_per_img:(
        i+1)*seq_per_img, attr_len[i]+att_len[i]:attr_len[i]+att_len[i]+rela_len[i], :]
...
class Decoder(nn.Module):
  ...
  def forward(self, x, memory_att, memory_attr, memory_rela, mask_att, mask_attr, mask_rela, tgt_mask):
    for layer in self.layers:
      x = layer(x, memory_att, memory_attr, memory_rela,
                mask_att, mask_attr, mask_rela, tgt_mask)
    return self.norm(x)
...
class DecoderLayer(nn.Module):
  ...
  def forward(self, x, memory_att, memory_attr, memory_rela, mask_att, mask_attr, mask_rela, tgt_mask):
    ...
    x_att = self.sublayer[1](x, lambda x: self.src_attn_att(
        x, memory_att, memory_att, mask_att))
    x_attr = self.sublayer[2](x, lambda x: self.src_attn_attr(
        x, memory_attr, memory_attr, mask_attr))
    x_rela = self.sublayer[3](x, lambda x: self.src_attn_rela(
        x, memory_rela, memory_rela, mask_rela))
...
def mod_controller(m_att, m_attr, m_rela, query):
  # m:50*17*512*3, query 50*17*512
  m = torch.stack((m_att, m_attr, m_rela), dim=3)
  d_k = query.size(-1)
  query = query.unsqueeze(-1)
  scores = torch.matmul(query.transpose(-2, -1), m) / \
      math.sqrt(d_k)  # scores:50*17*3
...
class TSGMModel3(AttModel):
  ...
  def _prepare_feature_forward(self, att_feats, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len, seq=None):
    ...
    att_feats = self.att_embed(att_feats) + self.type_embed(
        torch.ones(att_feats.shape[0:2], dtype=torch.int32).cuda()*0)
    rela_feats = self.rela_embed(rela_seq)

    for i in range(len(rela_len)):
      rela_type = self.type_embed(torch.ones(
          att_feats[i, att_len[i]:att_len[i]+attr_len[i], :].shape[0:1], dtype=torch.int32).cuda())
      att_feats[i, att_len[i]:att_len[i]+attr_len[i], :] = rela_feats[i,
                                                                      :attr_len[i], :] + rela_feats[i, attr_len[i]:2*attr_len[i], :] + rela_type
      att_feats[i, att_len[i]+attr_len[i]:att_len[i]+attr_len[i]+rela_len[i], :] = rela_feats[i, 2*attr_len[i]:2*attr_len[i]+rela_len[i], :] + \
          self.type_embed(torch.ones(att_feats[i, att_len[i]+attr_len[i]:att_len[i] +
                          attr_len[i]+rela_len[i], :].shape[0:1], dtype=torch.int32).cuda()*2)
  ...
  def _forward(self, fc_feats, att_feats, seq, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len):
    ...
    att_feats, seq, att_masks, seq_mask, enc_self_masks = self._prepare_feature_forward(
        att_feats, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len, seq)

    out = self.model(att_feats, seq, enc_self_masks, att_masks,
                     seq_mask, rela_len, attr_len, att_len)
  ...
  def _sample(self, fc_feats, att_feats, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len, opt={}):
    ...
    att_feats, att_masks, seq_mask, enc_self_masks = self._prepare_feature(
        att_feats, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len)
    ...
    for t in range(self.seq_length + 1):
      ...
      if remove_bad_endings and t > 0:
        tmp = logprobs.new_zeros(logprobs.size())
        prev_bad = np.isin(
            seq[:, t - 1].data.cuda().numpy(), self.bad_endings_ix)
  ...
  def _sample_beam(self, fc_feats, att_feats, att_masks, enc_self_masks, rela_seq, rela_len, attr_len, att_len, opt={}):
    ...
    logprobs, state = self.get_logprobs_state(
        it, att_feats, att_masks, state, rela_len, attr_len, att_len)
    ...
    self.done_beams = self.beam_search(
        state, logprobs, rela_len, attr_len, att_len, att_feats, att_masks, opt=opt)
  ...
  def get_logprobs_state(self, it, att_feats, att_masks, state, rela_len, attr_len, att_len, output_logsoftmax=1):
    ...
    output, state = self.core(
        xt, att_feats, state, att_masks, rela_len, attr_len, att_len)
